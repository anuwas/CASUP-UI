...
private baseUrl = '';
  ...
  getDevItemAllActivityStatus(devItem: String): Observable<any> {
    return this.http.get(`${this.baseUrl}/dev-item-status/${devItem}`);
  }

  saveDavItemCurrentStatus(item: object): Observable<object> {
    return this.http.post(`${this.baseUrl}`+'/save-item-daily-status', item);
  }
