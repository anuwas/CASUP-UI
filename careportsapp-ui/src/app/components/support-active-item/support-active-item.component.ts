...
export class SupportActiveItemComponent implements OnInit {
  ...
  activeIncidents: Observable<any>;
  activeServiceRequest : Observable<any>;
  continueServiceRequestData: Observable<any>;
  itemProblemRecordData: Observable<any>;
  itemActiveSupItemObj : Observable<any>;
  ...
  currentAPO : any;
  allSupUserNameList : any = [];
  allApplicationNameList : any = [];

  ngOnInit() {
    this.getActiveIncident('Incident');
    this.getActiveServiceRequest('Service Request');
    this.getContinueServiceRequest();
    this.getActiveProblemRecordPage();
    this.getPageConfigurationData();
  }

  getActiveIncident(itemType: string){
  this.spinner.show();
  this.supitemservice.getActiveItemLlistByType(itemType).subscribe(data =>{
        this.activeIncidents = data;
        this.spinner.hide();
    })
  }

  getActiveServiceRequest(itemType: string){
  this.spinner.show();
  this.supitemservice.getActiveItemLlistByType(itemType).subscribe(data =>{
        this.activeServiceRequest = data;
        this.spinner.hide();
    })
  }  

  getContinueServiceRequest(){
  this.spinner.show();
  this.supitemservice.getActiveReportItemList().subscribe(data =>{
        this.continueServiceRequestData =data;
        this.spinner.hide();
    })
  }
...
getActiveItemByType(itemType: string){
  this.spinner.show();
  this.supitemservice.getActiveItemLlistByType(itemType).subscribe(data =>{
        //console.log(data);
        this.itemActiveSupItemObj = data;
        this.spinner.hide();
    })
  }
  ...
  selectedTabChangeClick(event){
  //console.log(event.tab.textLabel);
    this.getActiveItemByType(event.tab.textLabel);
  }

  getPageConfigurationData(){
    let allSupUsers = GlobalConstants.getAllSupUsers();
    let allApplications = GlobalConstants.getAllApplicationNames();
    for(let allSupUsersObj of allSupUsers){
      this.allSupUserNameList.push({'value':allSupUsersObj,'title':allSupUsersObj});
    }
    for(let allApplicationsObj of allApplications){
      this.allApplicationNameList.push({'value':allApplicationsObj,'title':allApplicationsObj});
    }
  }
  ...
  actions: {
    ...
    columns: {
      ...
      applicationName: {
        ...
        config: {
                  selectText: 'Show All',
                  list: this.allApplicationNameList
                }
        },
        filter: {
        type: 'list',
