...
import { Devitemrequest } from '../../dto/devitemrequest';
...
allDevSprintNameList : any=[];
allDevUserNameList : any=[];
  ...
  currentSprintRedirect(){
    this.router.navigate(['/current-sprint']);
  }
  allSprintRedirect(){
    this.router.navigate(['/all-sprint']);
  }
    ...
    this.spinner.show();
     ...
     getRemoveItem(itemid){
      if(confirm("Are you sure to delete this Item "+itemid)) {
        ...
        this.devitemService.deleteItem(itemid)
            .subscribe(data => {
            this.getPage(this.config.currentPage);
        });
        }
 ...
 get DevItemId(){
  ...
  get ItemSubTaskSRC(){
    ...
    loadPageConfiguration(){
      ...
      this.allDevSprintNameList=GlobalConstants.getAllDevSprintNames();
      this.allDevUserNameList=GlobalConstants.getAllDevUsers();
    }
